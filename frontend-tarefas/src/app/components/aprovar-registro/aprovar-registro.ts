...
export class AprovarRegistroComponent implements OnInit {
  loading = true;
  success = false;
  error = false;
  title = '';
  ...
  ngOnInit() {
    ...
    if (token) {
      this.aprovarRegistro(token);
    } else {
      this.showError('Token inválido');
    }
  }

  aprovarRegistro(token: string) {
    this.http.get(`http://localhost:3000/tarefas/aprovar/${token}`)
      .subscribe({
        next: (response: any) => {
          this.loading = false;
          this.success = true;
          this.title = '✅ Registro Aprovado!';
          this.message = response.mensagem;
          this.usuario = response.usuario;
        },
        error: (error) => {
          this.loading = false;
          this.showError(error.error?.erro || 'Erro ao processar aprovação');
        }
      });
  }
